...
pub(crate) fn generate_eslint_migrate(mode: Mode) -> Result<()> {
    ...
    for (eslint_name, (group_name, rule_metadata)) in visitor.0 {
        ...
        let check_inspired = if is_inspuired {
            quote! {
                if !options.include_inspired {
                    results.inspired_rules.push(#eslint_name);
                    return false;
                }
            }
        } else {
        ...
        let check_nursery = if group_name == "nursery" {
            quote! {
                if !options.include_nursery {
                    results.nursery_rules.push(#eslint_name);
                    return false;
                }
            }
        } else {
        ...
        lines.push(quote! {
            #eslint_name => {
                ...
                results.migrated_rules.push(#eslint_name);
            }
        });
    }
    let tokens = xtask::reformat(quote! {
        ...
        pub(crate) fn migrate_eslint_any_rule(
            ...
            options: &eslint::MigrationOptions,
            results: &mut eslint::MigrationResults,
        ) -> bool {
            match eslint_name {
                #( #lines )*
                _ => {
